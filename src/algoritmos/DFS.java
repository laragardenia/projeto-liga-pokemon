...
public class DFS {
    ...
    //metodo adicionado para ser chamado na main
    public boolean ehConexo(Grafo grafo) {
        if (grafo == null || grafo.getQuantidadeVertices() == 0) {
            return false;
        }

        //pega o primeiro vértice qualquer do grafo para servir de ponto de partida
        Vertice inicial = grafo.getTodosVertices().iterator().next();
        
        //busca em profundidade a partir deste vértice inicial
        return verificarConexidade(grafo, inicial);
    }
    ...
    public boolean verificarConexidade(Grafo grafo, Vertice inicial) {
        ...
        visitar(grafo, inicial); //inicia o percurso de froma recursiva 
